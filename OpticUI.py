...
def iris2(canvas, pos, size, tint, radius=10, shadow_enabled=True, rounded=0, shadow_size=0.08, alpha=255, resolution=20, anti_glitch=False):
    global fancy, mode
    intensity = int((alpha / 255) * 30)
    if intensity > 255:
        intensity = 255
    ...
    if fancy == True:
        s.blit(canvas, [0, 0], [pos[0] - smooth_pad, pos[1] - smooth_pad, size[0] + smooth_pad * 2, size[1] + smooth_pad * 2])#, special_flags=pygame.BLEND_RGBA_ADD)
        s = pygame.transform.rotozoom(s, 0, (resolution / 100.0) * 1)
        size2 = s.get_size()
        if tint != False:
            pygame.gfxdraw.filled_polygon(s, [[0, 0], [size2[0], 0], size2, [0, size2[1]]], tint + [intensity])
        if mode == "light":
            pygame.gfxdraw.filled_polygon(s, [[0, 0], [size2[0], 0], size2, [0, size2[1]]], [255, 255, 255] + [int(70 * (alpha / 255))])
    ...
    if fancy == True:
        ...
        if mode == "light":
            pygame.gfxdraw.filled_polygon(canvas, [pos,
                                               [pos[0] + size[0], pos[1]],
                                               [pos[0] + size[0], pos[1] + size[1]],
                                               [pos[0], pos[1] + size[1]]], [255, 255, 255] + [int(255 * (alpha / 255))])
        else:
            pygame.gfxdraw.filled_polygon(canvas, [pos,
                                               [pos[0] + size[0], pos[1]],
                                               [pos[0] + size[0], pos[1] + size[1]],
                                               [pos[0], pos[1] + size[1]]], [50, 50, 50] + [int(255 * (alpha / 255))])
        """
        pygame.gfxdraw.filled_polygon(canvas, [pos,
                                               [pos[0] + size[0], pos[1]],
                                               [pos[0] + size[0], pos[1] + size[1]],
...
def drop_shadow(canvas, surface, posit, radius=2, alpha=255, resolution=100):
    ...
    s = pygame.Surface([size[0] + int((100 / resolution) * radius * 3), size[1] + int((100 / resolution) * radius * 3)], pygame.SRCALPHA)
    ...
    canvas.blit(b, [posit[0] - int(int((100 / resolution) * (radius / 2))), posit[1] - int((100 / resolution) * int(radius / 2))])
